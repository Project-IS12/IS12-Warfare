...
/obj/machinery/computer/account_database/Topic(href, href_list)
	...
	if(href_list["choice"])
		switch(href_list["choice"])
			...
			if("add_funds")
				var/amount = input("Enter the amount you wish to add", "Add funds") as num|null
				if(detailed_account_view)
					if(station_account.money >= amount)
						station_account.money -= amount
						detailed_account_view.money = min(detailed_account_view.money + amount, fund_cap)

			if("remove_funds")
				var/amount = input("Enter the amount you wish to remove", "Remove funds") as num|null
				if(detailed_account_view)
					station_account.money += amount
