	...
	if(istype(W, /obj/item/ammo_magazine))
		...
		if(caliber == A.caliber && src.BB)
			if(A.stored_ammo.len >= A.max_ammo)
				...
				A.update_icon()
				//user.visible_message(SPAN_NOTICE("\The [user] adds \a [src] to [A]."), SPAN_NOTICE("You add \a [src] to [A]."))
	...
	if(istype(W, /obj/item/ammo_magazine))
		...
		if(caliber == A.caliber)
			if(!A.stored_ammo.len)
				...
				update_icon()
				A.update_icon()
				//user.visible_message(SPAN_NOTICE("\The [user] adds \a [C] to [src]."), SPAN_NOTICE("You add \a [C] to [src]."))
