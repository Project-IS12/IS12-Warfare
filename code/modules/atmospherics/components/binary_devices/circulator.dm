...
/obj/machinery/atmospherics/binary/circulator/update_icon()
	if(stat & (BROKEN|NOPOWER) || !anchored)
		...
		if(last_pressure_delta > 5*ONE_ATMOSPHERE)
			icon_state = "circ-run"
			if(inverted)
				icon_state = "circi-run"
		else
			icon_state = "circ-slow"
			if(inverted)
				icon_state = "circi-slow"
	else
		icon_state = "circ-off"
		if(inverted)
			icon_state = "circi-off"
	return 1
...
/obj/machinery/atmospherics/binary/circulator/proc/rotate_clockwise()
	set category = "Object"
...
/obj/machinery/atmospherics/binary/circulator/RightClick(mob/user)
...
/obj/machinery/atmospherics/binary/circulator/AltClick(var/mob/user)
	if(!anchored)
		inverted = !inverted
		update_icon()
