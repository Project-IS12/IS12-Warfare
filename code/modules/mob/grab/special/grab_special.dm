...
/obj/item/grab/special/init()
	...
	if(assailant.zone_sel.selecting == BP_THROAT)
		grab_string = "throat"
	visible_message(SPAN_WARNING("[assailant] grabs [affecting]'s [grab_string]!"))
...
/datum/grab/special/wrench/proc/do_wrench(var/obj/item/grab/G)
	...
	if(!O)
		...
		return
	if(!G.wielded)
		to_chat(assailant, SPAN_WARNING("We must wield them in both hands to break their limb."))
		return
	...
	if(!O.is_broken()) // The limb is broken and we're grabbing it in both hands.
		assailant.visible_message(SPAN_DANGER("[assailant] tries to break [affecting]'s [O.name]!"))
