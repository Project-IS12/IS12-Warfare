...
/datum/grab/proc/handle_resist(var/obj/item/grab/G)
	...
	if(affecting.incapacitated(INCAPACITATION_KNOCKOUT | INCAPACITATION_STUNNED))
		to_chat(G.assailant, SPAN_WARNING("You can't resist in your current state!"))
	...
	if(break_strength < 1)
		to_chat(G.assailant, SPAN_WARNING("You try to break free but feel that unless something changes, you'll never escape!"))
		return
