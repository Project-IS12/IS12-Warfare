...
/mob/living/carbon/human/proc/morph()
	...
	visible_message("<span class='notice'>\The [src] morphs and changes [get_visible_gender() == MALE ? "his" : get_visible_gender() == FEMALE ? "her" : "their"] appearance!</span>", SPAN_NOTICE("You change your appearance!"), SPAN_WARNING("Oh, god!  What the hell was that?  It sounded like flesh getting squished and bone ground into a different shape!"))
		...
		for(var/obj/item/O in stomach_contents)
			if(O.edge || O.sharp)
				if(prob(1))
					...
					if(can_feel_pain())
						to_chat(src, SPAN_DANGER("You feel something rip out of your stomach!"))
						groin.embed(O)
				else if(prob(5))
...
/mob/living/carbon/human/verb/lookup()
	...
	if(!is_physically_disabled())
		...
		if(istype(above, /turf/space))
			to_chat(src, SPAN_NOTICE("You can see empty space - wow."))
		else
			to_chat(src, SPAN_NOTICE("There is nothing to see up there."))
	else
...
/mob/living/carbon/human/can_inject(var/mob/user, var/target_zone)
	...
	if(affecting.robotic >= ORGAN_ROBOT)
		to_chat(user, SPAN_WARNING("That limb is robotic."))
		return 0
...
/mob/living/carbon/human/proc/exam_self()
	...
	for(var/obj/item/organ/external/org in organs)
		...
		if((chem_effects[CE_PAINKILLER] < hurts))
			switch(hurts)
				if(1 to 49)
					status += SPAN_DANGER("<small>pain</small>")
				if(50 to 89)
					status += SPAN_DANGER("PAIN")
				if(90 to INFINITY)
					status += SPAN_DANGER("<big>PAIN</big>")
		if(org.robotic >= ORGAN_ROBOT)
			switch(org.damage)
				if(1 to 25)
					status += SPAN_DANGER("<small>slightly damaged</small>")
				if(26 to 49)
					status += SPAN_DANGER("damaged")
				if(50 to 99)
					status += SPAN_DANGER("VERY DAMAGED")
				if(100 to INFINITY)
					status += SPAN_DANGER("<big>BARELY WORKING</big>")



		for(var/datum/wound/wound in org.wounds)
			if(wound.embedded_objects.len)
				status += SPAN_DANGER("SHRAPNEL")
			if(wound.bandaged)
		...
		if(org.status & ORGAN_MUTATED)
			status += SPAN_DANGER("MISSHAPEN")
		if(org.status & ORGAN_BLEEDING)
			status += SPAN_DANGER("BLEEDING")
		if(org.dislocated == 2)
			status += SPAN_DANGER("DISLOCATED")
		if(org.status & ORGAN_BROKEN)
			status += SPAN_DANGER("BROKEN")
		if(org.splinted)
			status += "<span class='binfo'>SPLINTED</span>"
		if(org.status & ORGAN_DEAD)
			status += SPAN_DANGER("NECROTIC")
		if(org.is_dislocated()) //!org.is_usable() ||
			status += SPAN_DANGER("UNUSABLE")
		if(status.len)
...
/mob/living/carbon/human/RightClick(mob/living/user)
	...
	if(!user.Adjacent(src))
		...
		switch(intent)
			if(I_HURT)
				user.visible_message(SPAN_DANGER("<b>[user] threatens [src] with a fist!</b>"))
				return
			if(I_HELP)
...
/atom/AltRightClick(var/mob/living/carbon/human/user)
	...
	if(istype(user.loc, /turf/simulated/floor/trench))//We're in a trench.
		...
		if(trench_check >= 4)//But, if we're surrounded on all sides by trench, we cannot zoom in.
			to_chat(user, SPAN_WARNING("I'm too far away from the side to peer over."))
			return //ez pz
