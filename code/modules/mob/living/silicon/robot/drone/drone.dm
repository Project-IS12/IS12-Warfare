...
/mob/living/silicon/robot/drone/emag_act(var/remaining_charges, var/mob/user)
	...
	if(controlling_ai)
		to_chat(src, SPAN_DANGER("\The [user] loads some kind of subversive software into the remote drone, corrupting its lawset but luckily sparing yours."))
	else
...
/mob/living/silicon/robot/drone/proc/law_resync()
	...
	if(controlling_ai)
		to_chat(src, SPAN_WARNING("Someone issues a remote law reset order for this unit, but you disregard it."))
		return
...
/mob/living/silicon/robot/drone/proc/shut_down()
	...
	if(controlling_ai)
		to_chat(src, SPAN_WARNING("Someone issues a remote kill order for this unit, but you disregard it."))
		return
