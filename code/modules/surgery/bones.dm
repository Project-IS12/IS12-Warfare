...
/datum/surgery_step/set_bone/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	if (affected.status & ORGAN_BROKEN)
		user.visible_message(SPAN_NOTICE("[user] sets the [bone] n place with \the [tool]."), \
			SPAN_NOTICE("You set the [bone] in place with \the [tool]."))
		affected.stage = 2
	else
		user.visible_message(SPAN_NOTICE("[user] sets the [bone]</span> <span class='warning'>in the WRONG place with \the [tool]."), \
			SPAN_NOTICE("You set the [bone]</span> <span class='warning'>in the WRONG place with \the [tool]."))
		affected.fracture()
