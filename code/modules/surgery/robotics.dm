...
/datum/surgery_step/robotics/repair_burn/can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	if(..())
		...
		if(istype(C))
			if(!C.can_use(10))
				to_chat(user, SPAN_DANGER("You need ten or more cable pieces to repair this damage."))//usage amount made more consistent with regular cable repair
...
/datum/surgery_step/robotics/install_mmi/can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	if(!(affected.robotic >= ORGAN_ROBOT))
		to_chat(user, SPAN_DANGER("You cannot install a computer brain into a meat body."))
		return SURGERY_FAILURE
