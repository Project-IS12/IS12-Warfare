...
/obj/item/organ/internal/liver/proc/handle_thirst()
	...
	switch(owner.thirst)
		...
		if(0 to THIRST_LEVEL_DEHYDRATED)
			...
			if(prob(5))
				to_chat(owner, SPAN_WARNING("You faint from dehydration."))
				owner.Paralyse(5)
			else if(prob(6))
				to_chat(owner, SPAN_WARNING("You fall down because of your thirst."))
				owner.Weaken(1)
				owner.Stun(1)
			if(prob(10))
				to_chat(owner, SPAN_WARNING("You lick around your mouth as a craving for water sets in."))
				take_damage(1)