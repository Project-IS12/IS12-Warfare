		...
		if(ishuman(user))
			...
			if(H.isChild())
				if(!istype(src, /obj/item/grenade/smokebomb))
					to_chat(H, SPAN_WARNING("I'm just not able to pull the pin on \the [name]!"))
					return
		if(clown_check(user))
...
/obj/item/grenade/proc/activate(mob/user as mob)
	...
	if(user)
		log_and_message_admins("[user.name] ([user.ckey]) primed \a [src] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")
