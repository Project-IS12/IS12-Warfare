#define TANK_RELEASE_PRESSURE 24
...
/obj/item/tank/examine(mob/user)
	...
	if(valve_welded)
		to_chat(user, SPAN_WARNING("\The [src] emergency relief valve has been welded shut!"))
...
/obj/item/tank/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
	...
	data["defaultReleasePressure"] = round(TANK_RELEASE_PRESSURE)
...
/obj/item/tank/OnTopic(user, href_list)
	if (href_list["dist_p"])
		if (href_list["dist_p"] == "reset")
			distribute_pressure = TANK_RELEASE_PRESSURE
...
/obj/item/tank/proc/check_status()
	...
	if(pressure > TANK_FRAGMENT_PRESSURE)
		...
		if(integrity <= 0)
			...
			visible_message("\icon[src] <span class='danger'>\The [src] flies apart!</span>", SPAN_WARNING("You hear a bang!"))
			T.hotspot_expose(air_contents.temperature, 70, 1)
