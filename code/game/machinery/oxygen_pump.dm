...
/obj/machinery/oxygen_pump/attack_hand(mob/user as mob)
	...
	if(breather)
		...
		src.visible_message(SPAN_NOTICE("\The [user] makes \The [contained] rapidly retracts back into \the [src]!"))
		if(breather.internals)
			breather.internals.icon_state = "internal0"
		breather = null
		use_power = 1
...
/obj/machinery/oxygen_pump/proc/settings()
	set category = "Object"
	set name = "Show Tank Settings"
	ui_interact(usr)

/obj/machinery/oxygen_pump/RightClick(mob/user)
