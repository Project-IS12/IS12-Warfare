...
/decl/turf_initializer/maintenance/InitializeTurf(var/turf/simulated/T)
	...
	if(prob(vermin_probability))
		if(prob(80))
			new /obj/random/mob/roaches(T)///mob/living/simple_animal/mouse(T)
		else
			new /mob/living/simple_animal/hostile/retaliate/roach(T)///mob/living/simple_animal/lizard(T)
