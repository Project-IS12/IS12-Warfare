...
/turf/simulated/floor/plating/AltClick(var/mob/user)
	if(CanPhysicallyInteract(user))
		if(istype(user.get_active_hand(), /obj/item/stack/rods))
			...
			visible_message(SPAN_NOTICE("[user] has started place reinforced plating!"))
			if(R.get_amount() > 10)
...
/turf/simulated/floor/plating/reinforced/attackby(obj/item/C as obj, mob/user as mob)
	if(isWelder(C) && reinforced_stage == 1)
		...
		if(welder.isOn())
			visible_message(SPAN_NOTICE("[user] start to unweld lattice from [src]!"))
			if(do_after(user, 15 SECONDS) && welder.isOn())
				to_chat(user, SPAN_NOTICE("You unweld lattice from plating."))
				playsound(src, 'sound/items/Welder.ogg', 80, 1)
				overlays.Cut()
				reinforced_stage++
	if(isCrowbar(C) && reinforced_stage == 2)
		visible_message(SPAN_NOTICE("[user] start to pry off [src.name] from floor!"))
