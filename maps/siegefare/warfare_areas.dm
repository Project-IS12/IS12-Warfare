...
/area/warfare/battlefield
	...
	music = 'sound/siege/siegestorm.ogg'
...
/area/warfare/siegezones/outdoors
	music = 'sound/siege/siegestorm.ogg'
	icon_state = "storage"

/area/warfare/siegezones/outdoors/wavewall
	icon_state = "security"

/area/warfare/siegezones/indoors
	icon_state = "surgery"

/area/warfare/siegezones/outdoors/Entered(mob/living/L,  atom/A)
	. = ..()
	if(istype(L) && !istype(A, /area/warfare/siegezones))//Doesn't work but this does stop the lag.
		L.overlay_fullscreen("siege_fog", /obj/screen/fullscreen/siegefog)
		L.overlay_fullscreen("fallout", /obj/screen/fullscreen/fallout)
		L.overlay_fullscreen("ash", /obj/screen/fullscreen/storm)
		L.overlay_fullscreen("siege_storm", /obj/screen/fullscreen/siegestorm)

/area/warfare/siegezones/indoors/Entered(mob/living/L, area/A)
	. = ..()
	if(istype(L) && !istype(A, /area/warfare/siegezones))
		L.clear_fullscreen("siege_fog")
		L.clear_fullscreen("ash")
		L.clear_fullscreen("fallout")
		L.clear_fullscreen("siege_storm")
...
/area/warfare/battlefield/no_mans_land/Enter(atom/movable/mover as mob|obj, atom/forget as mob|obj|turf|area)
	if(!iswarfare())
		return TRUE
	if(ishuman(mover))
		...
		if(istype(SSjobs.GetJobByTitle(H.job), /datum/job/soldier/blue_soldier) && captured != H.warfare_faction)
